...
// ==========================================
// 1. SAFE MODEL DEFINITIONS (Crash Prevention)
// ==========================================
// We define Resource FIRST so Negotiation can reference it without crashing
...
const transporter = nodemailer.createTransport({
  ...
  // ⚠️ THIS FIXES THE RENDER/CLOUD SSL CRASH
  tls: {
...
// ==========================================
// 3. THE ROUTE (Simplified to prevent crashes)
// ==========================================
router.post('/send-approvals', async (req, res) => {
    ...
    try {
        ...
        // REMOVED .populate() to prevent DB relationship crashes
        ...
        // Send mails separately to catch individual errors
        ...
        res.status(500).json({ 
            error: "SERVER_ERROR", 
            message: err.message, // This sends the specific reason to your frontend
            stack: err.stack 
        });
    }
});
