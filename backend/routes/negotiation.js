...
// ==========================================
// 1. DEFINE SCHEMA (Robust for Vercel/Serverless)
// ==========================================
// We use 'mongoose.models.Negotiation || ...' to prevent OverwriteModelError in serverless env
...
router.post('/next-turn', async (req, res) => {
    try {
        ...
        // STOP LOGIC (Matching Localhost + Paid Status)
        ...
        try {
            ...
            let phasePrompt = isTransportPhase 
                ? `CURRENT PHASE: Logistics. Standard Cost: ₹${negotiation.transportCost}. JSON 'price' must be TRANSPORT COST. IF BUYER: Accept if asked before. IF SELLER: No discounts.`
                : `PHASE: Price. Ask: ₹${negotiation.currentSellerAsk}. Floor: ₹${negotiation.floorPrice}.`;
            ...
            // Fallback logic (Identical to your working code)
...
router.post('/send-approvals', async (req, res) => {
    try {
        ...
        // Default emails if missing (prevents crash)
        ...
        // ⚠️ VERCEL URL (Used for deployment)
        const baseUrl = 'https://omni-circulus.vercel.app'; 
        // NOTE: If you run this file locally, change to 'http://localhost:3000' manually or use env var
...
// --- VERIFY TRANSACTION (THE FIX) ---
router.post('/verify-transaction', async (req, res) => {
    // 1. Set CORS Headers specifically for this critical route
    ...
    try {
        ...
        console.log(`🔍 Verifying: Token=${token}, Action=${action}, Role=${role}`);
        ...
        if (['DEAL_CLOSED', 'PAID'].includes(negotiation.status)) {
            console.log("⚠️ Deal already closed");
            return res.json({ 
                success: true, 
                status: 'ALREADY_CLOSED', 
                message: "Deal already closed.", 
                negotiationId: negotiation._id // ID is CRITICAL for frontend redirect
            });
        }
        ...
        // 4. Update Approval Flags
        if (role === 'buyer') negotiation.buyerApproval = 'APPROVED';
        if (role === 'seller') negotiation.sellerApproval = 'APPROVED';

        console.log(`Update: Buyer=${negotiation.buyerApproval}, Seller=${negotiation.sellerApproval}`);

        // 5. Check Mutual Approval
        if (negotiation.buyerApproval === 'APPROVED' && negotiation.sellerApproval === 'APPROVED') {
            ...
            await negotiation.save();
            console.log("✅ Deal APPROVED. Sending response to frontend.");

            return res.json({ 
                success: true, 
                status: 'APPROVED', 
                negotiationId: negotiation._id // Frontend needs this to redirect to /payment/${id}
            });
        }

        // 6. Save Pending State
        await negotiation.save();
        console.log("⏳ Approval Recorded. Waiting for partner.");
        ...
        // Return 500 but as JSON so frontend doesn't just see "Network Error"
