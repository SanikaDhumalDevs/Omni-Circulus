...
router.post('/start', async (req, res) => {
    try {
        ...
        const negotiation = new Negotiation({
            ...
            turnCount: 0, 
            logs: [{ sender: 'SYSTEM', message: `CONNECTION ESTABLISHED. Item: ${resource.title}. Asking Price: ₹${resource.cost}.` }]
        });
...
router.post('/next-turn', async (req, res) => {
    try {
        ...
        try {
            ...
            const systemPrompt = `Act as ${currentAgent}. ${phasePrompt} Respond ONLY in JSON: { "action": "OFFER" | "ACCEPT" | "DECLINE", "price": number, "message": "string" } History: ${JSON.stringify(negotiation.logs.slice(-3))}`;
            ...
            if (isTransportPhase) {
                decision = currentAgent === 'BUYER_AGENT' 
                    ? { action: "ACCEPT", price: stdCost, message: "Okay, I accept." }
                    : { action: "OFFER", price: stdCost, message: "Standard rates apply." };
            } else {
...
// --- SEND APPROVALS (UPDATED to LOCALHOST) ---
router.post('/send-approvals', async (req, res) => {
    try {
        ...
        // ✅ FIX 1: CHANGE THIS TO LOCALHOST SO YOU CAN CLICK IT AND TEST
        const baseUrl = 'http://localhost:3000'; 
        // Note: When you deploy, change this back to: 'https://omni-circulus.vercel.app'
...
// This prevents the browser from blocking the connection before it even starts
...
router.post('/verify-transaction', async (req, res) => {
    // ✅ FIX 3: Force headers on the POST response too
    res.setHeader('Access-Control-Allow-Origin', '*'); 
    res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
    ...
    try {
        ...
        console.log(`🔍 Verifying Deal. Role: ${role}, Action: ${action}`);
...
router.get('/history/:email', async (req, res) => {
    try {
        ...
        }).sort({ updatedAt: -1 }).populate('resourceId', 'title cost location'); 
